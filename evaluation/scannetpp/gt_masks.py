...
def _load_cameras(repo_root, sparse_dir):
    ...
    result = []
    # Sort image names for deterministic output without changing camera poses
...
def _load_mesh(scene_root, metadata_path):
    ...
    for group in annotations["segGroups"]:
        ...
        # Map segment IDs to mesh vertices
...
def generate(scene_root, repo_root, metadata_path, output_dir, bands=4, viz=0,
    ...
    # Mark the mesh source so older outputs cannot be hit
    ...
    # Use the normalized COLMAP model
    ...
    for camera in cameras:
        ...
        for band in range(bands):
            ...
            # Transpose for row vector multiplication
        ...
        # Reassemble bands in the original image row order
        semantic = torch.cat(rendered_bands, dim=0).cpu().numpy()

        # Confidence follows the existing mask contract: every nonbackground
        # Represent every nonzero detector ID as a confident reference pixel
    ...
    # Add the vertical principal point
...
def main():
    ...
    generate(args.scene_root, args.repo_root, args.metadata, args.output_dir,
             args.bands, args.viz, args.force, args.mask_version,
             args.resolution)
